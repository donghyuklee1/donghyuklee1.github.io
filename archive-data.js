...
const ARCHIVE_POSTS = [
  {
    ...
    content: '<p>캠브리지 Lila Sciences 연구실을 소개한 MIT Technology Review 기사가 꽤 인상적이었어요. 진공 챔버에서 여러 원소를 스퍼터링해 박막을 만들고, AI 에이전트가 실험 설계와 최적화까지 맡는 모습이 나오더라고요. 알루미늄 팬에 촉매 후보들을 담은 샘플이 만들어지면, 다른 에이전트가 스캔해서 결과를 해석하고 다음 실험을 제안하는 식이에요. 아직은 사람이 최종 OK를 내리지만, 이 회사 입장에선 과학적 초지능으로 가는 첫걸음이라고 봅니다.</p><p>더 나은 전극, 이산화탄소 흡수 물질, 청정 수소용 촉매, 고온 초전도체—이런 소재들이 정말 필요한데, 소재 과학 쪽은 지난 수십 년간 상업적 성공 사례가 거의 없었어요. 난이도가 높고 성공률이 낮다 보니 생물학이나 신약 개발에 밀려 변방으로 밀려났죠.</p><img src="images/kaist 전경.jpeg" alt="연구 환경" class="archive-inline-image"><p>2020년 DeepMind의 AlphaFold2가 단백질 3D 구조 예측에 성공한 뒤, 2022년 ChatGPT가 등장하며 생성형 AI로 과학을 가속하겠다는 기대가 급증했습니다. 2023년에는 “수백만 개의 신규 소재를 발견했다”는 AI 모델 소식이 나왔고, 투자금이 쏟아졌습니다. 하지만 아직까지 ChatGPT 같은 돌파구는 없었습니다. 기적의 소재는 말할 것도 없고, 조금이라도 더 나은 소재도 실험적으로 입증되지 않았어요.</p><p>Lila Sciences의 John Gregoire는 핵심을 짚었더라고요. “시뮬레이션은 어떤 걸 실험할 가치가 있는지 이해하는 데 강력하다. 하지만 시뮬레이션만으로는 현실 문제를 해결할 수 없다.” 소재 발견에서 가장 오래 걸리고 비싼 단계는 새 구조를 상상하는 게 아니라, 실제로 만드는 것입니다. 합성하기 전에는 만들 수 있는지, 안정한지 알 수 없고, 많은 특성은 실험을 해봐야 비로소 알게 됩니다.</p><p>DeepMind의 2023년 논문은 “38만 개의 결정체가 가장 안정적”이라고 했지만, UC Santa Barbara 연구진은 “신규성·신뢰성·유용성을 모두 갖춘 화합물에 대한 근거가 거의 없다”고 반박했습니다. 일부는 이미 알려진 것의 사소한 변형에 불과했고, 절대영도에서 시뮬레이션한 결과는 상온 실험 조건과 맞지 않는 경우가 많았죠.</p><p>그래도 시뮬레이션과 실험 사이 간격은 조금씩 좁아지는 것 같아요. Periodic Labs처럼 DeepMind 출신이랑 ChatGPT 공동 창업자가 만든 스타트업은 계산 모델링 배경이 있는데도, 자동화된 실험실에서의 합성에 상당 부분을 건다고 하네요. LLM이 화합물 제조 레시피와 조건을 제안하고, 실험 데이터를 해석해서 다음 제안을 하는 식이에요. 궁극 목표 중 하나는 수십 년째 실패하고 있는 상온 초전도체—전기 저항 없이 흐르면서 실온에서 동작하는 소재를 찾는 거예요. 이 쪽 연구가 앞으로 더 본격화될 것 같습니다.</p>'
  },
  {
  ...
  },
  {
    ...
    content: '<p>ChatGPT가 2022년 말 등장한 이후 산업과 여러 국가 경제의 방향이 바뀌었습니다. 수백만 명이 컴퓨터와 대화하기 시작했고, 컴퓨터가 응답했죠. 우리는 매료됐고, 더 많은 것을 기대했습니다. 테크 기업들은 앞서가려 애썼고, 음성·이미지·비디오를 넘나들며 서로를 앞지르는 제품을 쏟아냈어요. 각 신제품이 주요 돌파구처럼 포장되며, 이 기술이 계속 나아질 것이라는 널리 퍼진 믿음이 굳어졌습니다. 그런데 2025년은 일종의 심판의 해가 되었어요.</p><p>상위 AI 기업 CEO들은 지키지 못한 약속을 남겼습니다. 생성형 AI가 화이트칼라 노동력을 대체하고, 풍요의 시대를 열며, 과학적 발견을 하고, 질병 치료제를 찾을 것이라고 했죠. FOMO에 휩쓸린 CEO들이 플레이북을 찢어버리고 AI에 뛰어들었어요. 그때부터 광택이 바래기 시작했습니다. 유럽연합 통계청, 스탠퍼드 대학 등 다양한 출처의 조사에 따르면 기업의 AI 도구 채택이 정체되고 있고, 많은 프로젝트가 파일럿 단계에 머물러 있어요. 넓은 경제 전반에서의 확산 없이는 대형 AI 기업이 이미 쏟아부은 엄청난 비용을 회수할 경로가 불분명해요.</p><p>동시에 핵심 기술 업데이트도 예전만큼의 도약이 아니에요. 가장 화제가 됐던 건 8월 GPT-5 런칭이었습니다. “어떤 분야든 박사급 전문가”라고 알트만이 호언했고, Death Star 이미지를 올려 기대를 부채질했죠. 그런데 나온 건—비슷한 것의 연속? “경계를 넘나드는 진보의 시대는 끝났다. AGI는 오지 않는다. 우리는 LLM의 삼성 갤럭시 시대에 있는 것 같다.” AI 연구자이자 유튜버 얀닉 킬처의 말이요. 스마트폰 비유가 적절해 보여요. 한동안 스마트폰이 가장 흥미로운 소비자 테크였지만, 지금은 애플이나 삼성의 신제품에 큰 화제가 없죠. 생성형 AI도 그렇게 되는 걸까요?</p><img src="images/IMG_8492.jpeg" alt="AI 밋업" class="archive-inline-image"><p>MIT Technology Review는 2025 말 AI의 상태를 생각하는 네 가지 관점을 제시해요. 첫째, LLM은 모든 것이 아니다. LLM이 AGI로 가는 관문이 아니라는 게 점점 분명해졌어요. OpenAI 전 수석 과학자 일리아 서츠케버도 LLM의 한계를 강조해요. 많은 구체적 과제를 수행하는 법은 배우지만, 그 과제들 뒤에 있는 원리를 배우는 것 같지는 않다는 거예요. 둘째, AI는 모든 문제의 빠른 해결책이 아니다. 7월 MIT 연구의 95% 수치는 “6개월 후에도 파일럿 단계를 넘지 못한 맞춤형 AI 도입 시도”를 기준으로 한 좁은 성공 정의를 반영해요. Andrej Karpathy가 말하듯, 챗봇은 많은 일에서 평균적인 인간보다 낫지만 전문가보다는 못해요. 그래서 개인 사용자에게는 유용하지만, 경제를 뒤집기에는 아직 부족한 거죠.</p><p>셋째, 버블이라면 어떤 종류인가. 2008 서브프라임처럼 붕괴 시 아무것도 남기지 않는 건가, 2000 닷컴처럼 많은 회사는 죽었지만 인터넷 인프라와 아마존·구글이 남은 건가. 아직 LLM의 킬러 앱이나 사업 모델이 뭔지 모르는 상황이에요. 넷째, ChatGPT는 시작도 끝도 아니다. 딥러닝은 80년대에 싹이 텄고, AI 연구는 50년대로 거슬러 올라갑니다. 그 기준으로 보면 생성형 AI는 이제 막 출발한 셈이죠. 하이프 시정은 오래 전부터 필요했지만, AI가 사라지지는 않을 것입니다.</p>'
  },
  {
  ...
  },
  {
    ...
    content: '<p>대부분의 LLM—GPT-4를 포함해—은 첫 번째로 떠오른 답을 그대로 출력해요. 가끔 맞고, 가끔 틀리죠. OpenAI의 o1, o3는 다르게 작동해요. 답을 단계별로 풀어가고, 막히면 다른 접근을 시도하는 훈련을 받았어요. 이 기법은 “추론(reasoning)”이라 불리는데—이 용어가 얼마나 논쟁적인지는 알지만—수학, 물리, 논리 문제에서 정확도를 크게 높아요. 에이전트에 있어서도 핵심이에요.</p><p>MIT Technology Review가 본 구글 DeepMind의 Mariner 에이전트 데모가 그 예시예요. 제품 매니저 메가 고엘이 사진에 나온 것처럼 생긴 크리스마스 쿠키 레시피를 찾아달라고 했어요. Mariner는 웹에서 레시피를 찾아 고엘의 온라인 장바구니에 재료를 담기 시작했습니다. 그런데 밀가루 종류에서 막혔어요. 어떤 밀가루를 골라야 할지 모르는 거죠. 그 순간 Mariner는 채팅 창에 “레시피로 돌아가기 위해 브라우저 뒤로가기 버튼을 사용하겠다”고 설명했습니다. 단순해 보이지만, 목적이 막혔을 때 작업을 세분화하고 뒤로 돌아가는 전략을 선택한 거예요. Mariner는 레시피로 돌아가 밀가루 종류를 확인한 뒤 장바구니 채우기를 이어갔습니다.</p><img src="images/IMG_5311.jpeg" alt="AI 밋업" class="archive-inline-image"><p>이건 LLM이 “생각”하는 방식을 보여주는 좋은 사례예요. 기존 모델이라면 막히는 순간 잘못된 것을 시도하거나 포기했을 가능성이 높아요. o1, o3 계열은 문제를 더 작은 부분으로 나누고, 한 경로가 통하지 않으면 대안을 시도하도록 학습했습니다. 구글은 Gemini 2.0의 실험 버전인 “Gemini 2.0 Flash Thinking”에서도 이 단계별 문제해결 방식을 적용하고 있어요.</p><p>OpenAI와 구글만이 아닙니다. 여러 회사가 비슷한 추론 기법을 쓰는 LLM을 만들고 있고, 요리부터 코딩까지 다양한 작업에서 성능이 올라가고 있어요. 2025년에는 “reasoning”에 대한 이야기가 훨씬 늘어날 겁니다. 다만 아직 이 기술이 왜, 어떻게 효과적인지에 대한 이해는 부족하고, 추론 단계가 길어질수록 토큰 비용과 지연 시간이 커지는 문제도 남아있어요. 그럼에도 o1, o3, Mariner 같은 데모는 LLM이 단순한 다음 토큰 예측기를 넘어, 다단계 계획과 적응이 가능한 시스템으로 변하고 있음을 보여줘요.</p>'
  }
];
