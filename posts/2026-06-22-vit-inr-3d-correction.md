...
The latent vector $z$ is explicitly disentangled into two orthogonal subspaces: auxiliary ($z_{aux}$) and density ($z_\rho$). To resolve spatial ambiguity, local coordinate evidence is extracted. The coordinate features are processed by a multi-layer perceptron conditioned via Feature-wise Linear Modulation (FiLM). The density head predicts the final bounded intensity field mapped into a valid range.
